...
public class MemberServiceImp implements MemberService {
	...
	public Member login(String id, String pass) {
		try {
			...
			if(user == null) throw new ProductException("등록되지 않은 아이디입니다.");
			
			if(!pass.equals(user.getPassword()))
				throw new ProductException("비밀 번호 오류 발생!!!!");
			return user;
		} catch (SQLException e) {
			e.printStackTrace();
			throw new ProductException("로그인 처리 중 오류 발생");
		}
	}
	@Override
	...
	public void regist(Member user) {
		try {
			...
			if(find!=null) {
				throw new ProductException("이미 등록된 아이디 입니다.");
			}
			dao.regist(user);
		} catch (SQLException e) {
			throw new ProductException("회원 정보 처리 중 오류 발생!!!");
		}
	}
